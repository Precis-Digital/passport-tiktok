...
Strategy.prototype.userProfile = function(accessToken, authData, done) {
    ...
    this._oauth2._request("GET", url, headers, null, null, function (err, body, res) {
 ...
 */
 Strategy.prototype.authorizationParams = function (options) {
    return {app_id: this.options.clientID};
};
...
Strategy.prototype.authenticate = function(req, options) {
	...
	if (req.query && req.query.code){
        function loaded(err, ok, state) {
            ...
            const tokenUrl = `${self.options.tokenURL}?app_id=${params.client_key}&secret=${params.client_secret}&auth_code=${code}`;
